...
После того как вы создадите удалённый репозиторий и выполните команды которые были написаны на сайте, вас попросят написать ваше имя, тут вы можете написать своё имя, но лучше указывать то которое вы указали в `user.name` или как в [github](https://github.com)

Но, после применения команды `git push`, git потребует ввести пароль, но это не пароль, а уникальный токен который можно сгенерировать с помощью [инструкции](GuideGithub.md#как-сгенерировать-свой-уникальный-токен) которую я уже написал
